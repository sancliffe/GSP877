...
# End-to-end gcloud automation script (Hardened Version 2)
...
# Prompt for Region/Zone if Qwiklabs hasn't set them in the environment
...
# --------------------------------------------------------------------------
# Task 2: Instance template + managed instance group
# --------------------------------------------------------------------------
echo ">>> Writing startup script to /tmp/startup-script.sh..."
...
if gcloud compute instance-templates describe "${TEMPLATE_NAME}" >/dev/null 2>&1; then
    ...
    # NOTE: deliberately global (no --region). The lab's console flow creates a
    # global template, and a zonal MIG referencing a *regional* template by bare
    # name is ambiguous/can fail. Keep this global unless you also fully-qualify
    # the template URL when creating the MIG below.
...
echo ">>> Setting named port on instance group"
...
if ! gcloud compute health-checks describe "${HEALTH_CHECK}" >/dev/null 2>&1; then
    gcloud compute health-checks create tcp "${HEALTH_CHECK}" \
      --port=80 --check-interval=5s --timeout=5s --healthy-threshold=2 --unhealthy-threshold=2
fi
...
if ! gcloud compute backend-services describe "${BACKEND_SERVICE}" --global >/dev/null 2>&1; then
    ...
    gcloud compute backend-services add-backend "${BACKEND_SERVICE}" \
      --instance-group="${MIG_NAME}" --instance-group-zone="${ZONE}" --global
fi
...
if ! gcloud compute url-maps describe "${LB_NAME}" >/dev/null 2>&1; then
    ...
    gcloud compute forwarding-rules create "${LB_NAME}-fw-rule" \
      --address="${LB_NAME}-ip" --global --target-http-proxy="${LB_NAME}-proxy" --ports=80
fi
...
# --------------------------------------------------------------------------
# Task 4: reCAPTCHA session-token and challenge-page site keys
# --------------------------------------------------------------------------
echo ">>> Setting up reCAPTCHA keys..."

# reCAPTCHA keys have generated resource IDs (no stable name to `describe`),
# so re-running the script used to always mint new keys. Look them up by
# display-name first so the script is safely re-runnable.
EXISTING_SESSION_KEY=$(gcloud recaptcha keys list \
  --filter="displayName=test-key-name" --format="value(name)" | head -n1 || true)
if [[ -n "${EXISTING_SESSION_KEY}" ]]; then
    SESSION_TOKEN_SITE_KEY=$(basename "${EXISTING_SESSION_KEY}")
    echo "    Session token key already exists: ${SESSION_TOKEN_SITE_KEY}"
else
    SESSION_KEY_OUTPUT=$(gcloud recaptcha keys create --display-name=test-key-name \
...
EXISTING_CHALLENGE_KEY=$(gcloud recaptcha keys list \
  --filter="displayName=challenge-page-key" --format="value(name)" | head -n1 || true)
if [[ -n "${EXISTING_CHALLENGE_KEY}" ]]; then
    CHALLENGE_PAGE_KEY=$(basename "${EXISTING_CHALLENGE_KEY}")
    echo "    Challenge page key already exists: ${CHALLENGE_PAGE_KEY}"
else
    CHALLENGE_KEY_OUTPUT=$(gcloud recaptcha keys create --display-name=challenge-page-key \
...
# --------------------------------------------------------------------------
# Push the HTML pages to the VM
# --------------------------------------------------------------------------
echo ">>> Waiting for managed instance group to stabilize..."
...
until gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --quiet --command "echo 'SSH Ready'" >/dev/null 2>&1; do
    ...
    if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
        echo "❌ Failed to connect via SSH after $MAX_RETRIES attempts. Please check VM status."
        exit 1
    fi
...
echo ">>> Building HTML pages locally..."
HTML_DIR=$(mktemp -d)
...
cat > "${HTML_DIR}/good-score.html" << 'EOF'
...
cat > "${HTML_DIR}/bad-score.html" << 'EOF'
...
cat > "${HTML_DIR}/median-score.html" << 'EOF'
...
echo ">>> Copying HTML pages to ${INSTANCE_NAME}..."
gcloud compute scp --quiet --zone "${ZONE}" \
  "${HTML_DIR}/index.html" "${HTML_DIR}/good-score.html" \
  "${HTML_DIR}/bad-score.html" "${HTML_DIR}/median-score.html" \
  "${INSTANCE_NAME}:/tmp/"

echo ">>> Installing HTML pages into /var/www/html..."
gcloud compute ssh "${INSTANCE_NAME}" --zone "${ZONE}" --quiet --command \
  "sudo mv /tmp/index.html /tmp/good-score.html /tmp/bad-score.html /tmp/median-score.html /var/www/html/"

rm -rf "${HTML_DIR}"
echo "    HTML pages deployed successfully."

# --------------------------------------------------------------------------
# Task 5: Cloud Armor security policy rules for Bot Management
# --------------------------------------------------------------------------
...
if ! gcloud compute security-policies describe "${SECURITY_POLICY}" >/dev/null 2>&1; then
    ...
    echo "    Policy ${SECURITY_POLICY} already exists. Skipping rule creation."
fi
